...
class UserModel
{
    ...
    public static function getPublicProfilesOfAllUsers()
    {
        ...
        $sql = "SELECT u.user_id, u.user_name, u.user_email, u.user_active, u.user_has_avatar, u.user_deleted, u.user_account_type, a.name AS account_type_name FROM users u LEFT JOIN account_types a ON u.user_account_type = a.id";
	...
	public static function getAllAccountTypes()
    {
        ...
        $sql = "SELECT id AS type_id, name AS type_name FROM account_types";
        $query = $database->prepare($sql);
        $query->execute();

        $all_account_types = array();

        foreach ($query->fetchAll() as $type) {
            array_walk_recursive($type, 'Filter::XSSFilter');

            $all_account_types[$type->type_id] = new stdClass();
            $all_account_types[$type->type_id]->type_id = $type->type_id;
            $all_account_types[$type->type_id]->type_name = $type->type_name;
        }
